<div align="center">
  <br />
    <a href="https://youtu.be/O5cmLDVTgAs?feature=shared" target="_blank">
      <img src="https://github.com/adrianhajdin/OneConnect/assets/151519281/a9cd1088-968b-4b1d-b21a-f5f97d0c202b" alt="Project Banner">
    </a>
  <br />
  ...
  <h3 align="center">A full stack OneConnect Clone</h3>
...
Build a full stack OneConnect clone using Next.js 14+ with a redesigned look transformed from a Figma design, user interaction to community management, technical implementation, and various features, including nested deep comments, notifications, real-time-search, and more.  
...
👉 **Visually Appealing Home Page**: A visually appealing home page showcasing the latest OneConnect for an engaging user experience.

👉 **Create Thread Page**: A dedicated page for users to create OneConnect, fostering community engagement

👉 **Commenting Feature**: A commenting feature to facilitate discussions within OneConnect.

👉 **Nested Commenting**: Commenting system with nested OneConnect, providing a structured conversation flow.
...
👉 **Admin-Specific Community OneConnect**: Enable admins to create OneConnect specifically for their community.

👉 **Community Search with Pagination**: A community search feature with pagination for exploring different communities.

👉 **Community Profiles**: Display community profiles showcasing OneConnect and members for a comprehensive overview.
...
```bash
git clone https://github.com/adrianhajdin/OneConnect.git
cd OneConnect
```
...
export async function deleteCommunity(communityId: string) {
  try {
    ...
    // Delete all OneConnect associated with the community
...
export const profileTabs = [
  { value: "OneConnect", label: "OneConnect", icon: "/assets/reply.svg" },
...
export async function fetchPosts(pageNumber = 1, pageSize = 20) {
  ...
  // Create a query to fetch the posts that have no parent (top-level OneConnect) (a thread that is not a comment/reply).
  ...
  // Count the total number of top-level posts (OneConnect) i.e., OneConnect that are not comments.
...
export async function createThread({ text, author, communityId, path }: Params
  ...
  try {
    ...
    await User.findByIdAndUpdate(author, {
      $push: { OneConnect: createdThread._id },
    });
...
export async function deleteThread(id: string, path: string): Promise<void> {
  try {
    ...
    // Fetch all child OneConnect and their descendants recursively
    ...
    // Recursively delete child OneConnect and their descendants
    await Thread.deleteMany({ _id: { $in: descendantThreadIds } });

    // Update User model
    await User.updateMany(
      { _id: { $in: Array.from(uniqueAuthorIds) } },
      { $pull: { OneConnect: { $in: descendantThreadIds } } }
    );
...
export async function fetchUserPosts(userId: string) {
  try {
    ...
    // Find all OneConnect authored by the user with the given userId
    const OneConnect = await User.findOne({ id: userId }).populate({
      path: "OneConnect",
      model: Thread,
      populate: [
        {
    ...
    console.error("Error fetching user OneConnect:", error);
    throw error;
  }
}
...
export async function getActivity(userId: string) {
  try {
    ...
    // Find all OneConnect created by the user
    ...
    // Find and return the child OneConnect (replies) excluding the ones created by the same user
    const replies = await Thread.find({
      _id: { $in: childThreadIds },
      author: { $ne: userId }, // Exclude OneConnect authored by the same user
    }).populate({
      path: "author",
      model: User,
...
export function formatThreadCount(count: number): string {
  if (count === 0) {
    return "No OneConnect";
  } else {
    const threadCount = count.toString().padStart(2, "0");
    const threadWord = count === 1 ? "Thread" : "OneConnect";
